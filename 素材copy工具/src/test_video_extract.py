...
def extract_video_features(video_path, target_fps=5, crop_ratio=0.2, quiet=False):
    """
    按指定帧率提取视频特征
    """
    if not quiet:
    ...
    total_target_frames = total_frames // frame_step
    ...
    pbar = None
    if not quiet:
        pbar = tqdm(total=total_target_frames, desc="特征提取", unit="帧", leave=False)
