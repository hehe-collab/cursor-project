/**
 * 自动化核心逻辑 - 3.0（Hooked Shorts / admin.hookedshorts.com）
 * smart+2.0 → 主体/账户 → 项目 → 广告组(Smart+2.0) → 广告(Smart+2.0) → 提交
 *
 * 基于 Element UI 组件选择器
 */
...
function isOnTaskPage(url) {
  return String(url || '').includes(config.paths?.task || '/batch-tools');
}

function isOnLoginPage(url) {
  return String(url || '').includes(config.paths?.login || '/login');
}

/** 尝试自动登录（admin.hookedshorts.com） */
async function tryAutoLogin(page) {
  if (!config.advanced?.autoLogin) return false;
  const user = String(config.username || '').trim();
  const pass = String(config.password || '').trim();
  if (!user || !pass) return false;

  log(`尝试自动登录: ${user}`, 'STEP');
  await page.goto(config.loginUrl || `${config.baseUrl}/login`, {
    waitUntil: 'domcontentloaded',
    timeout: 30000,
  });
  await mediumDelay();

  const userInput = page.locator('input[placeholder="用户名"], input[placeholder*="用户"]').first();
  ...
  try {
    await page.waitForFunction(
      () => !window.location.pathname.includes('/login') && !!localStorage.getItem('token'),
      { timeout: 15000 }
    );
  } catch {
    const errMsg = await page.locator('.el-message--error').first().innerText().catch(() => '');
    log(`自动登录失败${errMsg ? `: ${errMsg}` : ''}`, 'WARN');
    return false;
  }
...
/** 批量工具页是否已渲染（已登录且表单可见） */
async function isBatchToolsReady(page) {
  if (isOnLoginPage(page.url())) return false;
  try {
    await page.locator('button').filter({ hasText: '提交任务' }).first()
      .waitFor({ state: 'visible', timeout: 10000 });
    await page.locator('.filter-card .el-select').first()
      .waitFor({ state: 'visible', timeout: 10000 });
    return true;
  } catch {
    return false;
...
/** 导航到 batch-tools 并确保已登录、页面就绪 */
async function ensureLoggedInOnBatchTools(page) {
  await page.goto(config.taskUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
  // 等待 Vue 挂载批量工具表单
  await page.waitForSelector('.batch-tools-container, .filter-card', { timeout: 15000 }).catch(() => {});
  await mediumDelay();

  if (await isBatchToolsReady(page)) {
    log('批量工具页已就绪', 'OK');
    return true;
  }

  log('批量工具页未就绪，尝试自动登录...', 'WARN');
  let ok = await tryAutoLogin(page);
  if (ok) {
    await page.goto(config.taskUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
    await mediumDelay();
    if (await isBatchToolsReady(page)) return true;
  }

  log('清除可能过期的 token 并重试登录...', 'WARN');
  await page.evaluate(() => {
  ...
  log(`批量工具页仍未就绪，当前 URL: ${page.url()}`, 'ERROR');
  return false;
}

function getEntitySelectLocator(page) {
  // Hooked Shorts / Element Plus：placeholder 在 span 上，不在 input
  return page.locator('.filter-card .el-form-item').filter({
    has: page.locator('.el-form-item__label', { hasText: /^主体$/ }),
  }).locator('.el-select').first();
}

/** 调试日志占位（参考项目中的 agent log 已移除） */
function agentDebugLog() {}

async function debugAdGroupDialogSnapshot() {}
...
async function launchBrowser() {
  ...
  // 清除上次异常退出留下的 profile 锁，避免「profile already in use」
  ...
  const execPath = config.browser.executablePath
    ? path.resolve(__dirname, '..', config.browser.executablePath)
    : null;
  if (execPath && fs.existsSync(execPath)) {
    launchOptions.executablePath = execPath;
    log(`使用已有 Chromium: ${execPath}`, 'OK');
  } else if (execPath) {
    log(`配置的 Chromium 不存在: ${execPath}，将使用 PLAYWRIGHT_BROWSERS_PATH`, 'WARN');
  }
...
async function preparePage(page) {
  ...
  const ready = await ensureLoggedInOnBatchTools(page);
  if (!ready) {
    await pauseForUser(
      '无法自动进入批量工具页。请在浏览器中手动登录并打开 /batch-tools，然后按 Enter 继续'
    );
    await page.goto(config.taskUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
    await mediumDelay();
  }
  ...
  if (!(await isBatchToolsReady(page))) {
    log(`当前 URL: ${page.url()}，批量工具表单仍不可见`, 'WARN');
  }
  ...
  await pauseForUser(
    ...
    '   1. 如果需要登录，请手动登录\n' +
    '   2. 关闭所有弹窗（如"更改密码"提示等）\n' +
    '   3. 确保已进入「批量工具」页面（/batch-tools）\n' +
    '   4. 确认顶部为 Smart+2.0 模式\n' +
    '   准备好后按 Enter 开始执行任务'
  );

  if (!(await isBatchToolsReady(page))) {
    log('批量工具页未就绪，再次跳转...', 'WARN');
    await ensureLoggedInOnBatchTools(page);
  }
...
async function enableSmartPlus20PageToggle(page) {
  log('Step 1: 确认 Smart+2.0 模式（先于主体/账户）', 'STEP');
  await waitForLoadingMaskDisappear(page);
  try {
    // Hooked Shorts：默认 smart_plus；若为「常规手动广告」则点「切回 Smart+2.0」
    const switched = await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button, .el-button'));
      const backBtn = buttons.find((b) => /切回\s*Smart/i.test((b.textContent || '').trim()));
      if (backBtn) {
        backBtn.click();
        return 'button';
      }
      ...
      const alreadySmart = buttons.some((b) => /切换常规手动/i.test((b.textContent || '').trim()));
      if (alreadySmart) return 'already';
      return null;
    });

    if (switched === 'button') {
      await mediumDelay();
      log('已点击「切回 Smart+2.0」', 'OK');
    } else if (switched === 'switch') {
      await mediumDelay();
      log('smart+2.0 开关已开启', 'OK');
    } else if (switched === 'already') {
      log('已在 Smart+2.0 模式', 'OK');
    } else {
      log('未找到 Smart+2.0 切换控件，请人工确认页面模式', 'WARN');
    }
  } catch (err) {
    await pauseForUser(`确认 Smart+2.0 模式失败: ${err.message}\n请先手动切换再按 Enter`);
  }
}
...
async function selectEntity(page, entityName) {
  ...
  const select = getEntitySelectLocator(page);
...
function getAccountSelectLocator(page) {
  return page.locator('.filter-card .el-form-item').filter({
    has: page.locator('.el-form-item__label', { hasText: /^账户$/ }),
  }).locator('.el-select').first();
}
...
async function selectAccounts(page, accounts) {
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    const accountSelect = getAccountSelectLocator(page);
    await accountSelect.click();
    await shortDelay();

    // Element Plus 2.x：combobox input 可能在 is-hidden 容器内，用 force 或键盘输入
    const searchInput = accountSelect.locator('input.el-select__input').first();
    ...
    await shortDelay();
    await searchInput.fill(accountId, { force: true });
    await longDelay();
...
async function fillProjectNameV3(row, projectName) {
  ...
  await shortDelay();
  await nameInput.pressSequentially(String(projectName), { delay: 25 });
...
async function fillDailyBudgetProjectV3(row, budget) {
  ...
  if (!s || Number.isNaN(parseFloat(s.replace(/,/g, '')))) {
    log(`    每日预算: 为空，跳过（不限制项目预算；填 0 反而会让广告组进入 CBO 校验）`, 'INFO');
    return;
  }
...
async function setupProject(page, taskGroup) {
  ...
  // #region agent log — H-AU：在 setupProject 入口安装拦截器，覆盖项目阶段的所有 API
  await page.evaluate(() => {
    if (window.__agentNetHook) return;
    window.__agentNetHook = true;
    window.__agentReqs = [];
    ...
    window.fetch = function (input, init) {
      ...
      const entry = {
        ...
        body: typeof opts.body === 'string' ? String(opts.body).slice(0, 4000) : opts.body ? '[non-string-body]' : null,
        ts: Date.now(),
      };
      const p = origFetch(input, init);
      p.then(async (res) => {
        try { entry.status = res.status; entry.respPreview = String(await res.clone().text()).slice(0, 2000); }
        catch (e) { entry.respErr = String(e.message); }
        window.__agentReqs.push(entry);
      }).catch((e) => { entry.netErr = String(e.message); window.__agentReqs.push(entry); });
      return p;
    };
    const origOpen = XMLHttpRequest.prototype.open;
    const origSend = XMLHttpRequest.prototype.send;
    XMLHttpRequest.prototype.open = function (method, url) {
      this.__agentMethod = method; this.__agentUrl = url;
    ...
    XMLHttpRequest.prototype.send = function (body) {
      ...
      const entry = {
        kind: 'xhr',
        url: xhr.__agentUrl,
        method: (xhr.__agentMethod || 'GET').toUpperCase(),
        body: typeof body === 'string' ? String(body).slice(0, 4000) : body ? '[non-string-body]' : null,
        ts: Date.now(),
      };
      xhr.addEventListener('loadend', () => {
        entry.status = xhr.status;
        try { entry.respPreview = String(xhr.responseText || '').slice(0, 2000); } catch (e) {}
        window.__agentReqs.push(entry);
      });
  ...
  try {
    const clicked = await page.evaluate(() => {
      ...
      const projectCard = cards.find((c) => {
        const title = c.querySelector('.card-title');
        return title && title.textContent.trim() === '项目';
      });
      if (!projectCard) return false;
      const setBtn = Array.from(
        projectCard.querySelectorAll('.card-actions .el-button, .card-actions button, .el-button.is-link')
      ).find((b) => b.textContent.trim() === '设置');
      if (!setBtn) return false;
      setBtn.click();
      return true;
    });
    if (clicked) {
  ...
  if (!clickSuccess) {
    try {
      const projectSettings = page.locator('.tool-card')
        .filter({ has: page.locator('.card-title', { hasText: /^项目$/ }) })
        .locator('.el-button')
        .filter({ hasText: '设置' })
        .first();
      await projectSettings.click({ force: true });
      await mediumDelay();
      clickSuccess = true;
    } catch (err) {
      log(`Playwright 点击项目设置失败: ${err.message}`);
    }
  }
  ...
  const dialog = page.locator('[role=dialog]').filter({ hasText: '选择广告系列' }).first();
  ...
  // #region agent log — 项目弹窗确定前 dump：验证「每日预算」实际填了什么（H-T）
  try {
    const projDump = await dialog.evaluate((dlg) => {
      const trs = [...dlg.querySelectorAll('.el-table__body-wrapper .el-table__row')];
      return trs.map((tr, rIdx) => {
        const tds = [...tr.querySelectorAll('td')];
        return {
          rowIdx: rIdx,
          tds: tds.map((td, idx) => {
            const inputs = [...td.querySelectorAll('input.el-input__inner, input:not([type="hidden"])')];
            ...
            return {
              ...
              switchChecked: sw ? sw.classList.contains('is-checked') : null,
            };
          }),
        };
      });
    ...
    agentDebugLog({
      hypothesisId: 'H-T',
      location: 'automation.js:setupProject:beforeConfirm',
      message: 'project_pre_confirm_dump',
      data: { rows: projDump },
    });
  } catch (e) {
    agentDebugLog({
      hypothesisId: 'H-T',
      location: 'automation.js:setupProject:beforeConfirm',
      message: 'project_pre_confirm_dump_error',
      data: { err: String(e.message) },
    });
  }
  // #endregion
...
// ============================================================
//  3.0 广告组表（Smart+2.0）：账户(0) 项目(1) 广告组(2,空则新增,无需操作) 名称(3) Pixel(4) 优化目标(5) 出价(6) 广告组预算(7) 个数(8) 开始时间(9) 年龄(10) 商品库(11)
// ============================================================
...
async function ensureTabSmartPlus20(dialog) {
  ...
  if ((await tab.count()) === 0) {
    log('弹窗内无 Smart+2.0 标签，跳过（页顶已是 Smart+ 模式）', 'INFO');
    return;
  }
...
async function inputBidAdGroupV3(page, row, bid, optimizationTarget, count) {
  ...
  if (/[，；;、\r\n]/.test(bidStr)) {
    throw new Error(`出价「${bidStr}」格式错误：多个出价必须使用英文逗号 "," 分隔`);
  }
  const bidTokens = bidStr.split(',').map((s) => s.trim());
  if (bidTokens.some((s) => !s)) {
    throw new Error(`出价「${bidStr}」格式错误：英文逗号前后都必须有有效数字`);
  }
  const countStr = String(count ?? '').trim();
  const countNum = countStr ? Number(countStr) : 1;
  if (!Number.isInteger(countNum) || countNum <= 0) {
    throw new Error(`个数「${countStr}」格式错误：必须是大于 0 的整数`);
  }
  if (bidTokens.length !== countNum) {
    throw new Error(`出价个数(${bidTokens.length})与「个数」(${countNum})不一致`);
  }
  let normalizedTokens = bidTokens.map((token) => {
  ...
  const bidVal = normalizedTokens.join(',');
...
async function inputCountAdGroupV3(row, count) {
  ...
  if (Number(countStr) === 1 && current === '1') {
    log(`    个数: 保留默认 1（避免把前端数字类型改成字符串）`, 'OK');
    return;
  }
  await input.click();
  await input.fill('');
...
async function normalizeSmart2GroupModels(page) {
  const result = await page.evaluate(() => {
    const seen = new Set();
    function findVm(vm) {
      if (!vm || seen.has(vm)) return null;
      seen.add(vm);
      if (Array.isArray(vm.smart2Groups)) return vm;
    ...
    const roots = [];
    const appVm = document.querySelector('#app') && document.querySelector('#app').__vue__;
    if (appVm) roots.push(appVm);
    ...
    if (!vm) return { ok: false, reason: 'smart2Groups_vm_not_found' };

    const before = vm.smart2Groups.map((g) => ({
      price: g.price,
      groupTotal: g.groupTotal,
      groupTotalType: typeof g.groupTotal,
    }));

    vm.smart2Groups.forEach((g) => {
      const n = Number(g.groupTotal ?? 1);
      const normalized = Number.isFinite(n) && n > 0 ? n : 1;
      if (vm.$set) vm.$set(g, 'groupTotal', normalized);
      else g.groupTotal = normalized;
      if (g.price != null) {
        const price = String(g.price);
        if (vm.$set) vm.$set(g, 'price', price);
        else g.price = price;
      }
    });
  ...
  agentDebugLog({
    hypothesisId: 'H-GROUPTOTAL-TYPE',
    location: 'automation.js:normalizeSmart2GroupModels',
    message: 'normalize_smart2_group_models',
    data: result,
  });

  if (result.ok) {
    log(`  已修正 Smart+2.0 广告组个数类型为数字`, 'OK');
  } else {
    log(`  未能直接修正 Smart+2.0 广告组内部类型: ${result.reason}`, 'WARN');
  }
}
...
async function setupAdGroup(page, taskGroup) {
  ...
  try {
    const clicked = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('.tool-card'));
      const adGroupCard = cards.find((c) => {
        const title = c.querySelector('.card-title');
        return title && title.textContent.trim() === '广告组';
      });
      if (!adGroupCard) return false;
      const setBtn = Array.from(
        adGroupCard.querySelectorAll('.card-actions .el-button, .card-actions button, .el-button.is-link')
      ).find((b) => b.textContent.trim() === '设置');
      if (!setBtn) return false;
      setBtn.click();
      return true;
  ...
  if (!clickSuccess) {
    try {
      const adGroupSettings = page.locator('.tool-card')
        .filter({ has: page.locator('.card-title', { hasText: /^广告组$/ }) })
        .locator('.el-button')
        .filter({ hasText: '设置' })
        .first();
      await adGroupSettings.click({ force: true });
      await mediumDelay();
      clickSuccess = true;
    } catch (err) {
      log(`Playwright 点击广告组设置失败: ${err.message}`);
    }
  }
  ...
  const dialog = page.locator('[role=dialog]').filter({ hasText: '广告组设置' }).first();
  await dialog.waitFor({ state: 'visible', timeout: 10000 });
  log('广告组设置弹窗已打开');

  // #region agent log — H-AU：弹窗打开时 dump 项目阶段以来所有 task/* 请求
  try {
    const taskReqs = await page.evaluate(() => {
      const arr = (window.__agentReqs || []).slice();
      return arr
        .filter((r) => /\/task\//.test(r.url || ''))
        .map((r) => ({
          ts: r.ts,
          url: (r.url || '').split('?')[0],
          method: r.method,
          status: r.status,
          body: r.body,
          respPreview: (r.respPreview || '').slice(0, 600),
        }));
    });
    agentDebugLog({
      hypothesisId: 'H-AU',
      location: 'automation.js:setupAdGroup:onDialogOpen',
      message: 'task_reqs_so_far',
      data: { count: taskReqs.length, reqs: taskReqs.slice(-15) },
    });
  } catch (e) {
    agentDebugLog({
      hypothesisId: 'H-AU',
      location: 'automation.js:setupAdGroup:onDialogOpen',
      message: 'task_reqs_dump_error',
      data: { err: String(e.message) },
    });
  }
  // #endregion

  // #region agent log — H-AQ：在弹窗打开时安装 fetch/XHR 拦截器，捕获点确定时的网络流量
  await page.evaluate(() => {
    if (window.__agentNetHook) return;
    window.__agentNetHook = true;
    window.__agentReqs = [];
    ...
    window.fetch = function (input, init) {
      ...
      p.then(async (res) => {
        try {
          ...
          entry.respPreview = String(txt).slice(0, 2000);
        } catch (e) { entry.respErr = String(e.message); }
        window.__agentReqs.push(entry);
      }).catch((e) => { entry.netErr = String(e.message); window.__agentReqs.push(entry); });
      return p;
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    // #region agent log
    await debugAdGroupDialogSnapshot(page, dialog, row, `adgroup_before_fill_${i}`, i);
    // #endregion
    ...
    await withRetry(
      () => inputBidAdGroupV3(page, row, account.bid, account.optimizationTarget, account.count),
      `出价 [${account.bid}]`
    );
    await withRetry(
    ...
    await normalizeSmart2GroupModels(page);

    // #region agent log
    await debugAdGroupDialogSnapshot(page, dialog, row, `adgroup_after_fill_${i}`, i);
    // #endregion
  }

  // #region agent log — 真实 Vue 内部状态 & input meta（H-N）
  try {
    const vueDump = await dialog.evaluate((dlg) => {
      const tr = dlg.querySelector('.el-table__body-wrapper tr');
      if (!tr) return { ok: false, reason: 'no_tr' };
      function readVue(el) {
        const v = el.__vue__ || el.__vue_app__ || null;
        if (!v) return null;
        try {
          const data = v.$data || (v.proxy && v.proxy.$data) || {};
          return Object.keys(data).slice(0, 30).reduce((m, k) => {
            const val = data[k];
            if (val == null || ['string', 'number', 'boolean'].includes(typeof val)) m[k] = val;
            else if (Array.isArray(val)) m[k] = `Array(${val.length})`;
            else m[k] = `[${typeof val}]`;
            return m;
          }, {});
      ...
      return {
        ...
        tds: tds.map((td, idx) => {
          ...
          return {
            idx,
            txt: (td.innerText || '').replace(/\s+/g, ' ').trim().slice(0, 80),
            inputs: inputs.map((i) => ({
              value: i.value,
    ...
    agentDebugLog({
      hypothesisId: 'H-N',
      location: 'automation.js:setupAdGroup:beforeConfirm',
      message: 'pre_confirm_dump',
      data: vueDump,
    });
  } catch (e) {
    agentDebugLog({
      hypothesisId: 'H-N',
      location: 'automation.js:setupAdGroup:beforeConfirm',
      message: 'pre_confirm_dump_error',
      data: { err: String(e.message) },
    });
  }
  // #endregion
  ...
  // #region agent log
  try {
    const postConfirmMsgs = await page.locator('.el-message').allInnerTexts().catch(() => []);
    const postConfirmErr = await page.locator('.el-message--error').allInnerTexts().catch(() => []);
    const dialogStillVisible = await page
      .locator('[role=dialog]')
      .filter({ hasText: '广告组设置' })
      .count()
      .catch(() => -1);
    await page.waitForTimeout(1500);
    const dialogVisibleAfterWait = await page
      .locator('[role=dialog]')
      .filter({ hasText: '广告组设置' })
      .count()
      .catch(() => -1);
    const persistentErr = await page.locator('.el-message--error').allInnerTexts().catch(() => []);

    /** H-AQ：dump 拦截到的网络请求（仅留与广告组/校验/点确定相关的请求，且按时间倒序最近 8 条） */
    const recentReqs = await page.evaluate(() => {
      const all = (window.__agentReqs || []).slice();
    ...
    agentDebugLog({
      hypothesisId: 'H-E,H-AC,H-AQ',
      location: 'automation.js:setupAdGroup:afterConfirm',
      message: 'after_confirm_click',
      data: {
        postConfirmMsgs,
        postConfirmErr,
        dialogStillVisible,
        dialogVisibleAfterWait,
        persistentErr,
        recentReqs,
      },
    });
  ...
  if (dialogStillOpen > 0) {
    const errors = await page.locator('.el-message--error').allInnerTexts().catch(() => []);
    const messages = await page.locator('.el-message').allInnerTexts().catch(() => []);
    const reason = [...new Set([...errors, ...messages].map((s) => String(s).trim()).filter(Boolean))].join('；');
    throw new Error(`广告组设置弹窗未关闭${reason ? `：${reason}` : ''}`);
  }
...
async function selectPromotionLink(page, adGroupDialog, row, linkKeyword, opts = {}) {
  ...
  try {
    ...
    let linkDialog = page.locator('[role=dialog]').filter({ hasText: '选择推广链接' });
    try {
      await linkDialog.waitFor({ state: 'visible', timeout: 3000 });
    } catch {
      linkDialog = page.locator('[role=dialog]').last();
...
async function selectMaterials(
  ...
  try {
    ...
    // 方式1：找"已选"文字并点击（含 Hooked Shorts「已选 0 项」）
    let clicked = false;
    try {
    ...
    if (!clicked) {
      try {
        const materialCell = row.locator('td').nth(2); // 素材列
    ...
    // 等待素材选择弹窗（尝试多种标题）
    let materialDialog = page.locator('[role=dialog]').filter({ hasText: '选择素材' });
    try {
      await materialDialog.waitFor({ state: 'visible', timeout: 3000 });
    } catch {
      materialDialog = page.locator('[role=dialog]').last();
...
async function selectTitles(page, adGroupDialog, row, titles) {
  ...
  try {
    // 找到标题列的下拉选择器
    const cells = row.locator('td');
    const titleCell = cells.filter({ hasText: /请选择|可多选标题包|标题包/ }).first().or(cells.nth(3));
...
async function setupAdModalV3(page, taskGroup) {
  ...
  try {
    const clicked = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('.tool-card'));
      const adCard = cards.find((c) => {
        const title = c.querySelector('.card-title');
        return title && title.textContent.trim() === '广告';
      });
      if (!adCard) return false;
      const setBtn = Array.from(
        adCard.querySelectorAll('.card-actions .el-button, .card-actions button, .el-button.is-link')
      ).find((b) => b.textContent.trim() === '设置');
      if (!setBtn) return false;
      setBtn.click();
      return true;
  ...
  if (!clickSuccess) {
    try {
      const adSettings = page.locator('.tool-card')
        .filter({ has: page.locator('.card-title', { hasText: /^广告$/ }) })
        .locator('.el-button')
        .filter({ hasText: '设置' })
        .first();
      await adSettings.click({ force: true });
      await mediumDelay();
      clickSuccess = true;
    } catch (err) {
      log(`Playwright 点击广告设置失败: ${err.message}`, 'WARN');
    }
  }
  ...
  const dialog = page.locator('[role=dialog]').filter({ hasText: '广告设置' }).first();
  await dialog.waitFor({ state: 'visible', timeout: 10000 });
  log('广告设置弹窗已打开');
...
async function submitTask(page) {
  ...
  try {
    clickSuccess = await page.evaluate(() => {
      ...
      const submitButtons = allButtons.filter(btn => {
        ...
        return isVisible && (text === '提交' || text === '提 交' || text === '提交任务');
      });
  ...
  if (!clickSuccess) {
    try {
      const candidates = page.locator('button').filter({ hasText: /^(提交|提交任务)$/ });
