...
function parseBidTokens(val) {
  const raw = String(val ?? '').trim();
  if (!raw) return [];
  if (/[，；;、\r\n]/.test(raw)) {
    throw new Error(`出价「${raw}」格式错误：多个出价必须使用英文逗号 "," 分隔`);
  }
  const parts = raw.split(',');
  const tokens = parts.map((s) => s.trim());
  if (tokens.some((s) => !s)) {
    throw new Error(`出价「${raw}」格式错误：英文逗号前后都必须有有效数字`);
  }
  for (const token of tokens) {
    const n = Number(token);
    if (!Number.isFinite(n) || n <= 0) {
      throw new Error(`出价「${raw}」格式错误：「${token}」不是大于 0 的数字`);
    }
  }
  return tokens;
}

function getEffectiveAdGroupCount(val) {
  const raw = String(val ?? '').trim();
  if (!raw) return 1;
  const n = Number(raw);
  if (!Number.isInteger(n) || n <= 0) {
    throw new Error(`个数「${raw}」格式错误：必须是大于 0 的整数`);
  }
  return n;
}
...
function readTasks(filePath) {
  ...
  for (const [taskId, rows] of Object.entries(groups)) {
    ...
    for (const acc of accounts) {
      ...
      const bidTokens = parseBidTokens(acc.bid);
      const effectiveCount = getEffectiveAdGroupCount(acc.count);
      if (bidTokens.length !== effectiveCount) {
        throw new Error(
          `任务 ${taskId} 账户 ${acc.accountId}：出价个数(${bidTokens.length})与「个数」(${effectiveCount})不一致。` +
          `例如出价填 1.4,1.5,1.6 时，个数必须填 3`
        );
      }
      acc.bidCount = bidTokens.length;
      acc.effectiveCount = effectiveCount;
    }
...
function printTaskSummary(taskGroups) {
  ...
  for (const group of taskGroups) {
    ...
    console.log(
      `   优化目标: ${first?.optimizationTarget || '-'} | 出价: ${first?.bid || '-'} | 出价数: ${first?.bidCount || '-'} | 项目预算: ${first?.budget || '-'} | 广告组预算: ${first?.adGroupBudget || '-'} | 个数: ${first?.effectiveCount || '-'} | 基于目标增加预算: ${first?.increaseBudgetByGoalEnabled ? '开' : '关'}`
    );
