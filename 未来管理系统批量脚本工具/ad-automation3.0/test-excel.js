...
function testExcelData() {
  ...
  try {
    ...
    for (let i = 0; i < taskGroups.length; i++) {
      ...
      console.log(`  个数:           ${firstAcc?.effectiveCount || getEffectiveAdGroupCount(firstAcc?.count)}`);
      ...
      const bidTokens = parseBidTokens(bidStr);
      const effectiveCount = getEffectiveAdGroupCount(firstAcc?.count);
      const bidOk = bidTokens.length > 0;
      const bidCountOk = bidOk && bidTokens.length === effectiveCount;
      ...
      const checks = [
        ...
        {
          name: '出价个数/个数',
          value: `${bidTokens.length} 个出价 / 个数 ${effectiveCount}`,
          valid: bidCountOk,
        },
        {
          name: '项目预算/广告组预算',
