/**
 * DramaBagus 批量广告工具 - 主入口
 *
 * 使用方式：
 *   1. 先生成 Excel 模板：  npm run template
 *   2. 填写 data/tasks-dramabagus.xlsx
 *   3. 运行脚本：           npm start
 *   4. 浏览器会自动打开并尝试登录，进入批量工具页面
...
function printRoundSummary(successCount, failCount, total, failedTasks, extra = {}) {
  console.log('\n');
  console.log('╔══════════════════════════════════════════════════╗');
  console.log('║                  执行完毕                        ║');
  console.log('╚══════════════════════════════════════════════════╝');
  ...
  log(`📋  总计: ${total} 个`);
  if (extra.durationMinutes != null) {
    log(`⏱️  总耗时: ${extra.durationMinutes} 分钟`, 'INFO');
  }
}
  ...
  for (;;) {
    ...
    for (;;) {
      try {
        ...
        if (!context) {
          log('请先运行 npm run template 生成模板，然后填写 data/tasks-dramabagus.xlsx', 'INFO');
          process.exit(1);
        }
    ...
    if (config.parallel.enabled) {
      ...
      for (let i = 0; i < taskGroups.length; i++) {
        ...
        try {
          // 仅「进程第一轮 + 首条任务」不刷新；新一轮首任务需刷新以回到干净状态
          const isFirst = roundIndex === 1 && i === 0;
          await executeTaskGroup(page, taskGroup, i, taskGroups.length, isFirst);
          successCount++;
        } catch (err) {
          process.stdout.write('\x07');
