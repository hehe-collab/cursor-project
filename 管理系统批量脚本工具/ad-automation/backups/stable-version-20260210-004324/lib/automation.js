...
async function setupAdGroup(page, taskGroup) {
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    // Step 9-11: 选择推广链接
    await selectPromotionLink(page, dialog, row, account.linkKeyword);

    // Step 12-14: 选择素材（素材关键词优先，如果没有则使用素材ID）
    await selectMaterials(page, dialog, row, taskGroup.materialKeyword, taskGroup.materialIds);

    // Step 15: 选择标题（下拉多选 + 滚动）- 每个账户使用自己的标题
    await selectTitles(page, dialog, row, account.titles);

    // Step 16: 选择优化目标
    await selectOptimizationTarget(page, dialog, row, taskGroup.optimizationTarget);

    // Step 17: 输入出价和预算（传递优化目标用于风险控制）
    await inputBidAndBudget(page, dialog, row, taskGroup.bid, taskGroup.budget, taskGroup.optimizationTarget);

    // Step 18: 选择开始时间
    await selectStartTime(page, dialog, row, taskGroup.startDate, taskGroup.startTime);
...
async function selectPromotionLink(page, adGroupDialog, row, linkKeyword) {
  ...
  try {
    ...
    await pauseForUser(`选择推广链接失败: ${linkKeyword}\n错误: ${err.message}\n请手动选择`);
  }
}

// ── Step 12-14: 选择素材 ──

async function selectMaterials(page, adGroupDialog, row, materialKeyword, materialIds = []) {
  // 素材关键词优先，如果没有则使用素材ID
  ...
  try {
    ...
    if (rowCount === 0) {
      ...
      if (useKeyword) {
        ...
        if (!hasMatch) {
          log(`      ⚠️  前 ${checkCount} 行未检测到"${materialKeyword}"，但搜索返回了结果`, 'WARN');
          log(`      这可能是正常的（关键词在文件名深处），继续执行...`);
        }
      }
    ...
    await pauseForUser(`选择素材失败: ${materialKeyword}\n错误: ${err.message}\n请手动选择`);
  }
}
...
async function selectOptimizationTarget(page, adGroupDialog, row, target) {
  ...
  try {
    ...
    await pauseForUser(`选择优化目标失败: ${target}\n错误: ${err.message}\n请手动选择`);
  }
}
...
async function selectStartTime(page, adGroupDialog, row, date, time) {
  ...
  try {
    ...
    await pauseForUser(`设置开始时间失败: ${date} ${time}\n错误: ${err.message}\n请手动设置`);
  }
}
...
async function executeTaskGroup(page, taskGroup, index, total, isFirst = false) {
  ...
  // Step 1: 选择主体
  await selectEntity(page, taskGroup.entity);

  // 清除可能出现的弹窗
  await dismissPopups(page);

  // Step 2: 选择账户
  await selectAccounts(page, taskGroup.accounts);

  // 清除可能出现的弹窗
  await dismissPopups(page);

  // Steps 3-7: 推广项目设置（弹窗会在内部通过"确定"按钮关闭）
  await setupProject(page, taskGroup);
